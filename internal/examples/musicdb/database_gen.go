...
func (t ReadTransaction) GetAlbumsRowInterleaved(
	...
	it := t.ListAlbumsRowsInterleaved(ctx, ListQuery{
		Where: key.BoolExpr(),
		Limit: 1,
	})
	defer it.Stop()
...
func (t ReadTransaction) BatchGetAlbumsRowsInterleaved(
	...
	foundRows := make(map[AlbumsKey]*AlbumsRow, len(keys))
	if err := t.ListAlbumsRowsInterleaved(ctx, ListQuery{
		Where: spansql.Paren{Expr: where},
		Limit: int32(len(keys)),
...
func (t ReadTransaction) ListSongsRowsInterleaved(
	ctx context.Context,
	query ListQuery,
) *SongsRowIterator {
	if len(query.Order) == 0 {
...
func (t ReadTransaction) GetSongsRowInterleaved(
	ctx context.Context,
	key SongsKey,
) (*SongsRow, error) {
	it := t.ListSongsRowsInterleaved(ctx, ListQuery{
		Where: key.BoolExpr(),
		Limit: 1,
	})
	defer it.Stop()
...
func (t ReadTransaction) BatchGetSongsRowsInterleaved(
	ctx context.Context,
	keys []SongsKey,
) (map[SongsKey]*SongsRow, error) {
	if len(keys) == 0 {
	...
	foundRows := make(map[SongsKey]*SongsRow, len(keys))
	if err := t.ListSongsRowsInterleaved(ctx, ListQuery{
		Where: spansql.Paren{Expr: where},
		Limit: int32(len(keys)),
	}).Do(func(row *SongsRow) error {
		foundRows[row.Key()] = row
		return nil
	}); err != nil {
		return nil, err
